...
class Notice extends StatefulWidget {
  ...
  const Notice({Key? key, required this.club_uid, required this.club_name})
      : super(key: key);

  @override
  _NoticeState createState() => _NoticeState(club_uid, club_name);
}

class _NoticeState extends State<Notice> {
  String name = "";
  String club_uid = '';
  _NoticeState(this.club_uid, this.name);

  late int notice_leng;
  ...
  Future pickVideo() async {
    ...
    if (pickedFile != null) {
      ...
      info = await VideoCompress.compressVideo(_videoFile!.path,
  ...
  Future uploadVideoToFirebase(BuildContext context, String club_uid) async {
    fileName = basename(info!.file!.path);
    firebase_storage.Reference ref = firebase_storage.FirebaseStorage.instance
        .ref()
        .child("ClubProfile")
        .child(club_uid)
    ...
    firebase_storage.UploadTask uploadTask;
    //late StorageUploadTask uploadTask = firebaseStorageRef.putFile(_imageFile);
    uploadTask = ref.putFile(io.File(info!.file!.path), metadata);
  ...
  // Future _get_Notice_Post_leng() async {
  //   CollectionReference refWait_nomal_post = FirebaseFirestore.instance
  //       .collection("Club")
  //       .doc(name)
  //       .collection("Post");
  //   DocumentSnapshot snapshot_nomal_post_leng =
  //       await refWait_nomal_post.doc("NoticePost").get();

  //   notice_leng = snapshot_nomal_post_leng["NoticePost"].length;
  // }
                    ...
                    onPressed: () {
                      ...
                      try {
                        if (_imageFile != null) {
                          ...
                          uploadImageToFirebase(context, name);
                          ...
                          Timer(Duration(milliseconds: 2500), () async {
                            ...
                            FirebaseFirestore.instance
                                ...
                                .doc("NoticePost")
                                .update({
                              "NoticePost": FieldValue.arrayUnion(clubpostinfo),
                            });
                          ...
                          ///
                          /// 비디오 업로드 기능이지만 라이브러리 문제로 일단 보류
                          ///
                          String url = "";

                          uploadVideoToFirebase(context, name);
...
//                           "DateTime": formatDate(
//                               DateTime.now(), [yyyy, ".", mm, '.', dd]),
//                           "KeyNumber": notice_leng.toString(),
//                         });
