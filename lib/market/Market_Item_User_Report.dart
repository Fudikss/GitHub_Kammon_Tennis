...
class MarketItemUserReport extends StatefulWidget {
  final String prev_problem_user_uid;
  final String prev_problem_user_nick_name;

  const MarketItemUserReport({
    Key? key,
  ...
  @override
  _MarketItemUserReportState createState() => _MarketItemUserReportState(
        prev_problem_user_uid,
        prev_problem_user_nick_name,
      );
}

class _MarketItemUserReportState extends State<MarketItemUserReport> {
  ...
  bool _block_user = false;

  late UserAccountInfo userAccount_;

  _MarketItemUserReportState(
    this.problem_user_uid,
    this.problem_user_nick_name,
  );

  // Future _get_Block_User_id() async {
  //   DocumentReference ref =
  //       FirebaseFirestore.instance.collection("Users").doc(user1.uid);

  //   DocumentSnapshot snapshot = await ref.get();

  //   if (snapshot["BlockUserIDList"].length != 0) {
  //     for (int i = 0; i < snapshot["BlockUserIDList"].length; i++) {
  //       report_user_list.add(snapshot["BlockUserIDList"][i]);
  //     }
                    ...
                    if (report_check != true) {
                      try {
                        FirebaseFirestore.instance
                          ...
                          "ReporterNickName": userAccount_.nickname,
                        ...
                        FirebaseFirestore.instance
                            ...
                            .update({
                          "UserReportList":
                              FieldValue.arrayUnion([problem_user_uid]),
                        });
                        FirebaseFirestore.instance
                            .collection('Users')
                            .doc(problem_user_uid)
                            .update({
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        ListTile(
                            onTap: () {},
                            title: Text(
                              "${problem_user_nick_name}를 차단하시겠습니까?",
                              style: TextStyle(
                                  color: Colors.black,
                                  fontFamily: 'GSANSB',
                                  fontSize: 15.0),
                                ...
                                onChanged: (value) async {
                                  ...
                                  if (_block_user == true) {
                                    FirebaseFirestore.instance
                                        ...
                                        .doc(user1.uid)
                                        .update({
                                      "BlockUserIDList": FieldValue.arrayUnion(
                                          [problem_user_uid]),
                                    });
                                  } else {
                                    FirebaseFirestore.instance
                                        ...
                                        .doc(user1.uid)
                                        .update({
                                      "BlockUserIDList": FieldValue.arrayRemove(
                                          [problem_user_uid]),
                                    });
                                  }
                                })),
