...
import 'Market_Item_Buy_Info.dart';

class UserBuyList extends StatefulWidget {
  const UserBuyList({Key? key}) : super(key: key);

  @override
  _UserBuyListState createState() => _UserBuyListState();
}

class _UserBuyListState extends State<UserBuyList> {
  ...
  // Future _get_item_image_url() async {
  //   image_url.clear();
  ...
  //   Map<String, dynamic> data_list = result.data() as Map<String, dynamic>;

  //   for (int i = 0; i < data_list["BuyItem"].length; i++) {
  //     next_data.add(data_list["BuyItem"][i]["ImageUrl"][0]);
  //     doc_id.add(data_list["BuyItem"][i]["ItemID"]);
  //   }

  //   _get_storage_url(next_data, doc_id);
  // }

  // Future _get_storage_url(List url, List id_doc) async {
  //   List image_list = [];
  //   for (int i = 0; i < url.length; i++) {
  //     var storageReference = FirebaseStorage.instance
  //         .ref()
  //         .child("Market")
  //         .child(id_doc[i].toString())
  //         .child(url[i].toString());

  //     image_list.add(await storageReference.getDownloadURL());

  //     image_url.add(image_list[i].toString());
  //   }
  // }
            ...
            FutureBuilder(
                ...
                builder: (BuildContext context,
                  ...
                  if (snapshot.hasError) {
                    ...
                    if (data["BuyItem"].length != 0) {
                      for (int i = 0; i < data["BuyItem"].length; i++) {
                        item_title.add(data["BuyItem"][i]["Title"]);
                        item_intro.add(data["BuyItem"][i]["Intro"]);
                        item_price.add(data["BuyItem"][i]["Price"]);
                        item_datetime.add(data["BuyItem"][i]["DateTime"]);
                        item_nick_name.add(data["BuyItem"][i]["NickName"]);
                        item_user_uid.add(data["BuyItem"][i]["UserUID"]);
                        item_id.add(data["BuyItem"][i]["ItemID"]);
                        item_image_url.add(data["BuyItem"][i]["ImageUrl"]);
                        item_profile_image
                            .add(data["BuyItem"][i]["profile_image"]);
                      }
